...
let ticketPriceTotal = runners.reduce((acc,runner) => {
...
// Problem 1 The 5k wants to send gift baskets to the biggest donor's place of work. Create an array storing just the company name of donor's of 200 or more.
let highRollers = runners.filter((runner)=>runner.donation>200);
let businesses = [];
highRollers.forEach(person => {
  businesses.push(person.company_name);
});
